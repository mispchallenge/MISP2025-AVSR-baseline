        ...
        with ProcessPoolExecutor(num_jobs) as ex:
            if  channel_num!=1:
                ...
                for value in  recordings.values():
                    ...
                    dur_vals.append(min(tmptimes))  
                # dur_vals = ex.map(get_duration, recordings.values()+f".wav") #修改 ex.map(get_duration, recordings.values())
                    ...
                    Recording(
                        ...
                        sources= [
                            AudioSource(
                                ...
                                channels=[0],
                                source=path_or_cmd[:-1] # 原来这里么有f".wav"
                                if path_or_cmd.endswith("|")
                                else path_or_cmd+".wav",# 原来这里么有f".wav"
                            )
