...
from espnet2.asr.preencoder.gatecnn import GateCnn
...
class GatecnnfeatPreEncoder(AbsPreEncoder): #[B,T,D]->(B, 1, feat, T)->(B,512,1,T)->[B,T,512]
    def __init__(self,encoder_output_dim=512):
        super().__init__()
        self.model = GateCnn(encoder_output_dim=encoder_output_dim)

    def forward(self,x: torch.Tensor, lengths: torch.Tensor
    )-> Tuple[torch.Tensor, torch.Tensor]:
        max_seq_len = x.size(1)
        if max_seq_len % 64 != 0:
            x = torch.nn.functional.pad(x, pad=(0, 0, 0, 64 - max_seq_len % 64), mode='constant', value=0)
        x = x.transpose(1,2).unsqueeze(1)
        x = self.model(x)
        x = x.squeeze().transpose(1,2)
        lengths = (lengths.float() / 4).ceil().long()
        max_seq_len = lengths.max().int()
        return x[:,:max_seq_len,:], lengths
    def output_size(self) -> int:
        return  self.encoder_output_dim
...
if __name__ == "__main__":
    ...
    print(frontend)
    feats = torch.rand(16,64*5,80) 
    lengths = torch.randint(64*5-1,64*5,(16,))
