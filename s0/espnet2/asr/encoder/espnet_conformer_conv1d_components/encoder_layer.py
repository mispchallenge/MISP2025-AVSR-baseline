...
# For crosss-attention fusion encoder which innserted a cross attention to each conformer block
class CrossAttentionFusionEncoderLayer(nn.Module):
    ...
    def forward(self, x_input, mask, video, video_mask,cache=None):
        """Compute encoded features.
        ...
        if self.src_first:
            ...
            if pos_emb is not None:
                x_att = self.src_attn(video, x, x, pos_emb,mask)
            else:
                x_att = self.src_attn(video, x, x, mask)
