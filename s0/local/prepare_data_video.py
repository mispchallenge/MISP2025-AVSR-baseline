...
def prepare_wav_scp(file_pattern, store_dir):
    ...
    safe_store(file=os.path.join(store_dir, 'wav.scp'), data=sorted(all_file_lines), mode='add', ftype='txt')
    return None
...
def prepare_segments_text_utt2spk_worker(transcription_pattern, store_dir, processing_id=None, processing_num=None):    
    ...
    utt2spk_lines = []
    tier_name = '内容层'
    rejected_text_list = ['<其他说话人>', '<NOISE>', '<主说话人>', '<非会议内容>', '<DEAF>', '<knock>', '<telephonering>', '<sil>', '<cough>', '<soundofdoor>', '<movingsound>', '<laughter>', '<英文>', '<OVERLAP>', '<NOISE-MUSIC>', '<NOISE>*<NOISE>', '<NOISE>QU', '<aughter>'] + ['*'*i for i in range(1, 30)]
    punctuation_list = [' ','。','.','?','？','!','！','，',',','、']
    ...
    transcription_pool = []
    while file_ids:
        file_id_0 = file_ids.pop(0)
        file_id_120 = file_ids.pop(0)
        file_id_240 = file_ids.pop(0)
        file_id = file_id_0.replace('-0','')
        ...
        while transcription_files or transcription_pool:
            ...
            if file_id_split[0] in transcription_filename_split:
                transcription2id_dic[transcription_file] = file_id
            else:
        ...
        if processing_token:
            ...
            for interval in target_tier.intervals:
                if interval.text not in rejected_text_list and interval.xmax - interval.xmin >= min_duration:
                    ...
                    #utterance_id = 'S{}_{}'.format(speaker, '_'.join(transcription_filename.split('_'))) + '_' + \
                    utterance_id = 'S{}_{}'.format(speaker, '_'.join(transcription_filename.split('_')[:-2])) + '_' + \
                                    '{0:06d}'.format(int(round(start_stamp*100, 0))) + '_' + \
                                    '{0:06d}'.format(int(round(end_stamp*100, 0)))
                    text = interval.text
                    for punctuation in punctuation_list:
                        text = text.replace(punctuation, '')
                    if text not in sound_list and text not in rejected_text_list:
                        segments_lines.append('{} {} {} {}'.format(utterance_id, file_id+'-0', start_stamp, end_stamp))
                        segments_lines.append('{} {} {} {}'.format(utterance_id, file_id+'-120', start_stamp, end_stamp))
                        segments_lines.append('{} {} {} {}'.format(utterance_id, file_id+'-240', start_stamp, end_stamp))
...
def prepare_segments_text_utt2spk_manager(transcription_pattern, store_dir, processing_num=1):
    ...
    safe_store(file=os.path.join(store_dir, 'segments'), data=sorted([*set(segments_lines)]), mode='add', ftype='txt')
    safe_store(file=os.path.join(store_dir, 'text_sentence'), data=sorted([*set(text_sentence_lines)]), mode='add', ftype='txt')
    safe_store(file=os.path.join(store_dir, 'utt2spk'), data=sorted([*set(utt2spk_lines)]), mode='add', ftype='txt')
    return None
...
if __name__ == '__main__':
    ...
    print('Preparing segments,text_sentence with {}'.format(args.transcription_pattern))
