...
def text2lines(textpath, lines_content=None, **extra):
    """
    ...
    if lines_content is None:
        ...
        if extra['mode']=='add':
            with codecs.open(textpath, 'a') as handle:
...
def safe_store(file, data, mode='cover', ftype=None, **other_params):
    ...
    def store_process(filename, content, **extra):
        ...
        if file_type == 'npy':
            ...
            text2lines(textpath=filename, lines_content=content, mode=extra['mode'])
    ...
    if os.path.exists(file):
        if mode == 'ignore':
            ...
            # os.remove(file)
            store_process(filename=file, content=data, mode='cover', **other_params)
        elif mode == 'add':
            store_process(filename=file, content=data, mode='add', **other_params)
        else:
