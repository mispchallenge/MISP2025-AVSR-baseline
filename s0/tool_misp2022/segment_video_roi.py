...
def segment_video_roi_json(video_path, roi_json_path, roi_store_dir, segments_name, segments_start, segments_end, roi_type='lip', segments_speaker=None, roi_size=[96, 96], roi_sum=False):
...
def match_json_file(json_dir, filename):
    search = os.path.join(json_dir, f'{filename}*.json')
    json_file = glob.glob(search)
...
def input_interface(data_root, roi_json_dir, need_speaker=True, roi_type='head', roi_size=[96, 96], roi_sum=False):
    ...
    for video_line in [*map(lambda x: x[:-1] if x[-1] in ['\n'] else x, lines_content)]:
        ...
        video_dic[name] = path
        roi_json_dic[name] = match_json_file(roi_json_dir, os.path.splitext(os.path.split(path)[-1])[0])
