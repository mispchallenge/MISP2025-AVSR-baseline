...
class AVSRModel_MISP2022_VSR(ASRModel_MISP2022_ASR):
    """CTC Encoder-Decoder: VSR_MISP2022"""
    """
       这里用的是conv3d+resnet18和conformer[Wenet]
    """
    def __init__(
        self,
        vocab_size: int,
        ...
        super(ASRModel_MISP2022_ASR, self).__init__()
    ...
    def forward(
        ...
        """Frontend + Encoder-Decoder + Calc CTC loss
        ...
        # MISP2022: video_frontend + E-D
        # video_frontend
        # print(speech.shape, speech_lengths.shape)
        # torch.Size([8, 9, 96, 96, 3]) torch.Size([8])
        if self.video_frontend is not None:
            speech, speech_lengths = self.video_frontend(speech, speech_lengths)  #[B,T]->[B,T,D] 25fps
        # video_frontend
        # print(speech.shape, speech_lengths.shape)
        # torch.Size([8, 36, 512]) torch.Size([8])
        # 1. Encoder
        ...
        if self.ctc_weight != 0.0:
            # print(encoder_out.shape, text.shape, encoder_out_lens, text_lengths)
