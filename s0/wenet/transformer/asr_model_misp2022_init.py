...
class ASRModel_MISP2022(torch.nn.Module):
    """CTC-attention hybrid Encoder-Decoder: ASR_MISP2022"""
    def __init__(
        self,
        vocab_size: int,
    ...
    def forward(
        ...
        # torch.Size([8, 42, 80]) torch.Size([8])
        # torch.Size([8, 9, 256]) torch.Size([8, 1, 9])
        # torch.Size([8, 4]) torch.Size([8])
        # print(encoder_out_lens, text_lengths)
        # tensor([32, 31, 31, 31, 30, 30, 30, 30], device='cuda:0') tensor([ 7,  7,  7, 10,  7,  8,  7,  9], device='cuda:0', dtype=torch.int32)
        
        # Add Pre-processing CTCLoss要求 input_length >= 2*target_length + 1,否则会算出nan
        # tensor([21, 21, 21, 21, 21, 21, 21, 21], device='cuda:0') tensor([ 6,  4,  4,  3,  5, 11,  4,  4], device='cuda:0', dtype=torch.int32)
        reserve_location = []
        for i in range(encoder_out_lens.shape[0]):
            # encoder_out_lens[i]数据类型:tensor(14, device='cuda:0'),可用encoder_out_lens[i].item()转为14
            if (encoder_out_lens[i] >= (2*text_lengths[i]+1)):
                reserve_location.append(i)
    
        if len(reserve_location) != encoder_out_lens.shape[0]:
            # print(encoder_out, text)
            # print(encoder_out.shape, text.shape)
            # print(encoder_out_lens, text_lengths)
            # print(encoder_out.dtype, text.dtype)
            # print(encoder_out_lens.dtype, text_lengths.dtype)
            # print(encoder_mask.dtype)
            # torch.Size([8, 9, 256]) torch.Size([8, 4])
            # torch.float32 torch.int64
            # torch.int64 torch.int32
            # torch.bool
            reserve_location.sort()
            new_batch_size = len(reserve_location)
            encoder_out_new = torch.zeros((new_batch_size, encoder_out.shape[1], encoder_out.shape[2]))
            encoder_mask_new = torch.zeros((new_batch_size, encoder_mask.shape[1], encoder_mask.shape[2]))
            text_new = torch.zeros((new_batch_size, text.shape[1]))
            encoder_out_lens_new, text_lengths_new = torch.zeros(new_batch_size), torch.zeros(new_batch_size)

            encoder_out_new, text_new = encoder_out_new.to(encoder_out.device), text_new.to(text.device)
            encoder_out_lens_new, text_lengths_new = encoder_out_lens_new.to(encoder_out_lens.device), text_lengths_new.to(text_lengths.device)
            encoder_mask_new = encoder_mask_new.to(encoder_mask.device)
            for index in range(new_batch_size):
                encoder_out_new[index], text_new[index] = encoder_out[reserve_location[index]], text[reserve_location[index]]
                encoder_out_lens_new[index], text_lengths_new[index] = encoder_out_lens[reserve_location[index]], text_lengths[reserve_location[index]]
                encoder_mask_new[index] = encoder_mask[reserve_location[index]]
            encoder_out, text = encoder_out_new, text_new
            encoder_out_lens, text_lengths = encoder_out_lens_new, text_lengths_new
            encoder_mask = encoder_mask_new
            text, encoder_out_lens, text_lengths = text.long(), encoder_out_lens.long(), text_lengths.int()
            encoder_mask = encoder_mask.bool()
            # torch.Size([7, 9, 256]) torch.Size([7, 4])
            # torch.float32 torch.int64
            # torch.int64 torch.int32
            # torch.bool
            # text: tensor([], device='cuda:0', size=(0, 5), dtype=torch.int64)
    ...
    def _calc_att_loss(
        ...
        r_ys_pad = reverse_pad_list(text, ys_pad_lens, float(self.ignore_id))
    ...
    def recognize(
        ...
        for i in range(1, maxlen + 1):
            ...
            # logp: (B*N, vocab)
            # print(encoder_out.shape)  torch.Size([10, 22, 256])
            # print(hyps.shape)     torch.Size([10, 1])
            # print(encoder_mask.shape)   torch.Size([10, 1, 22])
            # print(hyps_mask.shape) torch.Size([10, 1, 1])
            logp, cache = self.decoder.forward_one_step(
                encoder_out, encoder_mask, hyps, hyps_mask, cache)
            # print(logp.shape) torch.Size([10, 3890])
        ...
        # print(scores) tensor([[-0.4629, -5.9574, -7.3179, -7.3213, -7.7654, -7.8978, -7.9386, -8.0504, -8.2898, -8.5330]], device='cuda:0')
        # TODO: length normalization
        best_scores, best_index = scores.max(dim=-1)
        # print(best_scores, best_index) tensor([-0.4629], device='cuda:0') tensor([0], device='cuda:0')
        best_hyps_index = best_index + torch.arange(
            batch_size, dtype=torch.long, device=device) * beam_size
        # print(best_hyps_index) tensor([0], device='cuda:0')
        best_hyps = torch.index_select(hyps, dim=0, index=best_hyps_index)
        best_hyps = best_hyps[:, 1:]
        # print(best_hyps, best_scores)
        # tensor([[1367, 1676,   53,  527, 3889, 3889, 3889, 3889, 3889]], device='cuda:0') tensor([-0.4629], device='cuda:0')
        # S016_R01_S016017_C02_I1_000156-000252 我是一号
