...
class AVSRModel_MISP2022_AVSR(torch.nn.Module):
    """CTC-attention hybrid Encoder-Decoder: AVSRModel_MISP2022[AVSR]"""
    def __init__(
        self,
        vocab_size: int,
    ...
    def forward(
        ...
        if self.video_frontend is not None:
            video, video_lengths = self.video_frontend(video, video_lengths)  #[B,T]->[B,T,D] 
        # 1. Encoder
        encoder_out, encoder_mask = self.encoder(speech, speech_lengths, video, video_lengths)
        encoder_out_lens = encoder_mask.squeeze(1).sum(1)
        # print(speech.shape, speech_lengths.shape)
        # print(encoder_out.shape, encoder_mask.shape)   # torch.Size([1, 2, 256]) torch.Size([1, 1, 2])
        # print(text.shape, text_lengths.shape)          # torch.Size([1, 1]) torch.Size([1])
        # print(encoder_out_lens, text_lengths)          # tensor([2], device='cuda:0') tensor([1], device='cuda:0', dtype=torch.int32)
        ...
        if self.ctc_weight != 0.0:
            # print(encoder_out.shape, encoder_mask.shape, encoder_out_lens)
    ...
    def ctc_prefix_beam_search(
        ...
        """
        hyps, _ = self._ctc_prefix_beam_search(speech, speech_lengths, video, video_lengths,
    ...
    def attention_rescoring(
        ...
        hyps, encoder_out = self._ctc_prefix_beam_search(
            speech, speech_lengths, video, video_lengths, beam_size, decoding_chunk_size,
