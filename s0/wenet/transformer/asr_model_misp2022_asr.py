...
class ASRModel_MISP2022_ASR(torch.nn.Module):
    """CTC-attention hybrid Encoder-Decoder: ASR_MISP2022"""
    """
       这里用的是conformer[Wenet]+contransformer
    """
    def __init__(
        self,
        vocab_size: int,
        global_cmvn: Optional[None],
    ...
    def forward(
        ...
        # MISP2022: 提特征->SpecAug->CMVN->Preencoder
        # Wenet中: ConformerEncoder有CMVN这个选项。
        # CMVN
        if self.global_cmvn is not None:
            speech = self.global_cmvn(speech)
        # print(speech.shape, speech_lengths.shape)
        # torch.Size([8, 38, 80]) torch.Size([8])
        
        # 0. Pre-encoder
        if self.preencoder is not None:
            if isinstance (self.preencoder, VGGfeatPreEncoder):
                speech, speech_lengths = self.preencoder(speech, speech_lengths)  #[B,T]->[B,T,D] 25fps
            else:
                pass
        # 1. Encoder
        # print(speech.shape, speech_lengths.shape)
        # torch.Size([8, 10, 512]) torch.Size([8])
        encoder_out, encoder_mask = self.encoder(speech, speech_lengths)
        encoder_out_lens = encoder_mask.squeeze(1).sum(1)
        # print(encoder_out.shape, encoder_out_lens.shape, encoder_mask.shape)
        ...
        if self.ctc_weight != 1.0:
            loss_att, acc_att = self._calc_att_loss(encoder_out, encoder_out_lens,
                                                    text, text_lengths)
        else:
            loss_att = None
    ...
    def _calc_att_loss(
        ...
        # Change:zhewang18 此处的解码用的是contransformer
        if isinstance(self.decoder, ConvTransformerDecoder):
    ...
    def recognize(
        ...
        # torch.Size([1, 24, 512]) torch.Size([1, 1, 24])
        ...
        for i in range(1, maxlen + 1):
            ...
            # logp: (B*N, vocab)
            # print(encoder_out.shape, encoder_mask.shape, hyps.shape, hyps_mask.shape)
            # torch.Size([10, 24, 512]) torch.Size([10, 1, 24]) torch.Size([10, 1]) torch.Size([10, 1, 1])
        ...
        # print(scores) tensor([[ -2.5707,  -7.4489,  -8.3054,  -9.4820, -10.0524, -10.8390, -11.1802, -11.2699, -12.4382, -15.0865]], device='cuda:0')
        # print(scores)
        ...
        # tensor([[3889, 3889, 3889, 3889, 3889, 3889, 3889, 3889, 3889, 3889, 3889, 3889, 3889, 3889, 3889, 3889, 3889, 3889, 3889, 3889, 3889, 3889, 3889, 3889]], device='cuda:0') tensor([-2.5707], device='cuda:0')
