...
import wenet.dataset.processor_misp2022 as processor
...
def Dataset_Visual(data_type,
    ...
    if data_type == 'shard':
        ...
        # Notice: 此处是parse_raw_visual函数
        dataset = Processor(dataset, processor.parse_raw_visual)
    ...
    if sort:
        sort_conf = conf.get('sort_conf', {})
        dataset = Processor(dataset, processor.sort_v, **sort_conf)

    batch_conf = conf.get('batch_conf', {})
    dataset = Processor(dataset, processor.batch, **batch_conf)
    dataset = Processor(dataset, processor.padding_v)
    return dataset


def Dataset_1kh(data_type,
                data_list_file,
                symbol_table,
                conf,
                bpe_model=None,
                non_lang_syms=None,
                partition=True):
    # 该函数用于1kh Audio Dataset, yaml中设置speed_perturb: false 
    """ Construct dataset from arguments
    ...
    if data_type == 'shard':
        ...
        dataset = Processor(dataset, processor.parse_raw_1kh)
...
def Dataset_Visual_Cat(data_type,
                       data_list_file,
                       symbol_table,
                       conf,
                       bpe_model=None,
    ...
    if data_type == 'shard':
        ...
        # Notice: 此处是parse_raw_visual函数
        dataset = Processor(dataset, processor.parse_raw_visual_cat)
