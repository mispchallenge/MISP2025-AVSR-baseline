...
from wenet.dataset.dataset_misp2022_avsr import Dataset_AV
from wenet.utils.checkpoint import (load_checkpoint, save_checkpoint,
                                    load_trained_modules, load_pretrain_modules)
from wenet.utils.executor_misp2022_avsr import Executor
from wenet.utils.file_utils import read_symbol_table, read_non_lang_symbols
from wenet.utils.scheduler import WarmupLR, NoamHoldAnnealing
from wenet.utils.config import override_config
from wenet.utils.init_model_misp2022_avsr import init_model_misp2022_avsr
...
def main():
    ...
    train_dataset = Dataset_AV(args.data_type, args.train_data, symbol_table,
                            train_conf, args.bpe_model, non_lang_syms, True)
    cv_dataset = Dataset_AV(args.data_type,
                         args.cv_data,
                         symbol_table,
                         cv_conf,
                         args.bpe_model,
    ...
    if 'fbank_conf' in configs['dataset_conf']:
        input_dim_a = configs['dataset_conf']['fbank_conf']['num_mel_bins']
    else:
        input_dim_a = configs['dataset_conf']['mfcc_conf']['num_mel_bins']
    vocab_size = len(symbol_table)

    # Save configs to model_dir/train.yaml for inference and export
    configs['input_dim_a'] = input_dim_a
    ...
    model = init_model_misp2022_avsr(configs)
    print(model)
    ...
    # REMOVE: torch.jit.script不支持enumerate(self.layers)
    # if args.rank == 0:
    ...
    init_param = configs['init_param']
    name_map = configs['name_map']
    # print(init_param)
    # print(name_map)
    ...
    elif (init_param is not None) and (name_map is not None):
        infos = load_pretrain_modules(model, init_param, name_map)
    else:
        infos = {}
